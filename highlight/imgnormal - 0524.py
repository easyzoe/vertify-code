...
def com_Neighbors_Deep(srcpath,dstpath='',datatype=0,file_moveflag = 0,debugflag = 0):
    
    if len(dstpath) == 0:
        dstpath = srcpath
    
    featlist=  [ fs_whole_hist,fs_obj_hist,fs_vex_hist,fs_edge_hist,fs_area,fs_lbp ]
    ...
    for i in range(num):
       ...
       res_color =  com_any_cluster( index_distance[fs_color][1],index_distance[fs_color][0],num,i,thre = 0.2)
       #res_area =  com_any_cluster( index_distance[fs_area][1],index_distance[fs_area][0],num,i,thre = 0.3)
       res_lbp =  com_any_cluster( index_distance[fs_lbp][1],index_distance[fs_lbp][0],num,i,thre = 0.2 )
       # split lbp  thre = 0.15; obj lbp 0.06  独立的obj 没有分割时，采用0.06
       res_hu =  com_any_cluster( index_distance[fs_hu][1],index_distance[fs_hu][0],num,i,thre = 0.2 )
...
def onegather_try1():
    ...
    srcpath = 'E:\\match_sample\\test\\1495635263-4\\'
