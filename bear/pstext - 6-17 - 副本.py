# -*- coding: utf-8 -*-
from skimage import io,filters,img_as_ubyte,img_as_float,draw,measure
...
def testfun(textimg):
   ...
   eg = feature.canny(textimg,sigma = 1.5 )
   
   morphology.binary_erosion(nozpos,mask) 
    
def erose_island(texteg):
    
    cen_col,cen_row =  compcenter(texteg)
    ...
    for lab in rgps:
        if lab.area /total_area  > 0.1:
            continue
        dis = np.linalg.norm( np.array(lab.centroid) - np.array([cen_row,cen_col]) )
        print dis ,lab.label,lab.area
        
        if dis  > 18:
           labels[labels == lab.label] = 0
           print "erose island"
...
def move_win_forword(texteg,wn = 2):
    size = 14
    num = texteg.shape[1]
    
    cptexteg = texteg.copy()
    ...
    ret = cv2.findContours(texteg.astype(np.uint8),0,2) 
    contours =ret[1]  
    mask = np.zeros(texteg.shape,np.uint8)    
    bfind = 0
    for ks in range(len(contours)):
       cnt = contours[ks]     
       rect = cv2.boundingRect(cnt)
       if rect[3] >= 14 and rect[2] >= 14  :
           print "get word"
           #word =  texteg[ rect[1]:rect[1]+rect[3],rect[0]:rect[0]+rect[2]  ] 
           
           cv2.drawContours(mask,[cnt],0,255,-1)
           #pixelpoints = np.transpose(np.nonzero(mask))
           
          
    word  =  np.multiply(texteg,mask)
    if word.sum() >0 :    
        rect = getMaxRect(word)
        
        if ( rect[3]- rect[2] ) >= wn*size:
            return word
          
    dtextedg = morphology.binary_dilation(texteg,morphology.disk(1))
    ret = cv2.findContours(dtextedg.astype(np.uint8),0,2) 
    contours =ret[1]  
    mask = np.zeros(texteg.shape,np.uint8)    
    for ks in range(len(contours)):
       cnt = contours[ks]     
...
def  newword_format(word):
    ...
    if np.abs(ang ) > 3:
       ...
       cen_col,cen_row =  compcenter(word)
       if np.abs(ang)  > 30:
          ram = cv2.getRotationMatrix2D((cen_col,cen_row),90+ang,1)
       else :
          ram = cv2.getRotationMatrix2D((cen_col,cen_row),ang,1)
       
       dst = cv2.warpAffine(word.astype(np.uint8),ram,(cols,rows))
...
def new_split_word(afile,wn):
    ...
    for word in words:
       ...
       # 利用 cv 旋转文字无效果
       word = newword_format(word)
       subf=  'E:\\picdog\\ctextout\\%s_%d_%d_ra.png'%(filename[:-4], n,wn)
       word =  word > 0
       #if gcom_debug():
       io.imsave(subf,word*255);
    ...
    atxeg = txeg.astype(np.uint8)
    #plt.imshow(atxeg)
    #plt.show()
    word = move_win_forword(txeg)
    
    abc = (txeg.sum(axis =0 )  >  5 )
    
    pos =  np.where(abc )[0]
    
    len = pos.max() - pos.min()
    median = np.median(pos)
    print os.path.basename(afile),  len, median
    ...
    ap =  cluster.KMeans(n_clusters=wn+1).fit(pt)
    print ap.labels_.max()
    labelpt = pt[np.where(ap.labels_ == 0)[0]]
    tmp = np.zeros(txeg.shape)
    tmp[labelpt[:,1],labelpt[:,0]] = 1
    
    
    
    ret = cv2.findContours(atxeg,0,2)  
    contours =ret[1] 
    print len(contours)
    cnts = [] 
    ...
    for ks in range(len(contours)):
       ...
       showimg = np.zeros(atxeg.shape)
       rr,cc = draw.polygon( cnt[:,0,1], cnt[:,0,0])
       showimg[rr,cc] =  1
       plt.imshow(showimg)
       plt.show()
       ...
       cntp = [ area,cx,cy]
       partlist.extend([cntp])
       
    partlist = np.array(partlist)
    #a1 = partlist[:,::-1].T
    #a2 = np.lexsort(a1)
    #sorted_partlist = partlist[a2]
    
    pt = partlist[:,1:]
    ap = KMeans(n_clusters=wn).fit(pt)
    
    cnlabel = np.array(range(len(contours)))
    for i in range(wn):
        parts = cnlabel[np.where(ap.labels_ ==  i)[0]]  
        img = yy.copy()
        img  = img*255
        for k in parts:
           cnt = contours[k]
        ...
        plt.imshow(img)
        plt.show()
       #img = cv2.drawContours(yy, contours, cnts[0], (0,255,0), 3)       
       #plt.imshow(img)
       #plt.show()   
    a = 0   
