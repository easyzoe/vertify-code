...
def cv_roateimage(vex,aimg):
    ...
    mask = np.zeros(vex.shape,np.float)    
    ...
    if np.abs( ang ) > 0:
       ...
       resflag = (maskdst > 0.96 )
       rrm,ccm = np.where(resflag > 0)
       
       dst[maskdst < 0.2 ] = 0 
       
       dst = dst[rrm.min():rrm.max()+1,ccm.min():ccm.max()+1]
...
def img_feature_grow(afile,debugflag=0):
    ...
    dst = None # = cv_roateimage(edge,cpimg)   
...
def com_judge_feature(aimgfeat,bimgfeat,featlist,debugflag = 0 ):
    ...
    for ft in featlist:
        ...
        elif ftype == fs_whole_hist: 
            dis = distance.cosine(aimgfeat[fs_whole_hist],bimgfeat[fs_whole_hist])   
...
def scan_test():
    ...
    srcpath = 'E:\\match_sample\\onecoarse\\image-1495415635-35\\'
    dstpath = 'E:\\match_sample\\testout\\'
    ...
    com_Neighbors_Deep(srcpath,dstpath=dstpath,datatype = 1,file_moveflag = 0,debugflag = 0,whole_thre = 0.05)
...
def com_class_num():
    srcpath = 'E:\\match_sample\\onecoarse\\'
...
def img_macro_feature_grow(afile,smoothflag =0,debugflag=0):
    aimg = io.imread(afile)
    
    return img_macro_feat(aimg,smoothflag =smoothflag)

def img_macro_feat(aimg,smoothflag=0):    
    imgfeat= {}
    ...
    gray_img = color.rgb2gray(cpimg)
    '''
    agray_img = img_as_ubyte(gray_img).astype(np.uint8)
    edge = cv2.Canny(agray_img,50,150)
    kernel = np.ones((5,5),np.uint8)
    edge = cv2.morphologyEx(edge, cv2.MORPH_CLOSE, kernel)
    '''
    
    edge = feature.canny(gray_img,sigma =0.8 )
    edge =  morphology.binary_closing(edge,morphology.square(5) )
    
    #edge = imgmarco_marrhildreth(gray_img,sigma=0.8)
    #plt.imshow(edge)
    #plt.show()
    feat_edge = edge #  move_tocenter(edge)  
    wholehist = com_color_hist(cpimg,fstype=fs_lab)
    
    label = measure.label(edge,neighbors=8)
    ...
    if label.max() > 1  :
        ...
        for b in range(1,label.max()+1 ):
            ...
            if label_num < tb.sum():
               label_num = tb.sum()
               max_label = b
        if  label_num > edge.sum()* 0.6:  # get max region
            vex = (label == max_label)      
    ...
    '''
    # 从edge 找最大轮廓
    rest = cv2.findContours((edge*1).astype(np.uint8),cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    ...
    timg = aimg.copy()      
    cnt = rest[1][maxindex]
    timg = cv2.drawContours(timg, [cnt], 0, (0,255,0), 3)
    ...
    imgfeat[fs_edge]       =  edge            # gray_img   # img_gray
    imgfeat[fs_whole_hist] = wholehist
    return imgfeat   
    

#wather
def img_macro_watershed(afile):
    img = cv2.imread(afile.encode('gbk'))
...
def img_macro_textfeat_tohdf5(srcpath,text,textid,h5f):
    os.chdir(srcpath)
    filelist = glob.glob(r'*.jpg')
    
    num =  len(filelist)
    
    textgrp = h5f.create_group(text)
   
    textgrp.create_dataset('num',(1,),data =(num)) 
    textgrp.create_dataset('filelist',(num,),data = filelist )
    
    for i in range(num):
        afile = os.path.join(srcpath,filelist[i] )
        aimgfeat = img_macro_feature_grow(afile)
        grp = textgrp.create_group(filelist[i])
        for k ,val in aimgfeat.items():
            if k == fs_edge:
              dty =  'u1'
            elif k == fs_gray:
...
def img_marco_baiduimg_hdf5(srcpath,yamlpath):
    h5fname =  os.path.join(srcpath,"feat.hdf5")
    ...
    fp = open( os.path.join(yamlpath, "coreconfig.yaml"),'r')
    config = yaml.load(fp)
    
    for textid,text in config['textlist'].items():
        subpath =  os.path.join(  srcpath  , "%s"%text )
        img_macro_textfeat_tohdf5(subpath,text,textid,f)
        
    f.close()
    return 

def img_macro_get_textfeat(srcpath,text):
    ...
    num = textgroup['num'].value
    filelist = textgroup['filelist'].value
    data = []
    for afile in filelist:
        afeat = com_readfeat_fromh5dy(textgroup,afile) 
        data.append(afeat)
